...
vector<int> solution(vector<int> answers)
{
    ...
    for(int i = 0; i < answers.size(); i++)
    {
        ...
        {
            int pattern_size = pattern[j].size();
            if (answer == pattern[j][i%pattern_size]) 
            {
                score[j]++;
            }
        }
...
// [1, 3, 2, 4, 2] [1, 2, 3]
    ...
    vector<int> input = {1,2,3,4,5}; // 정답 1
    //vector<int> input = {1,3,2,4,2}; // 정답 1,2,3

    vector<int> answers = solution(input);
